...
class Leg_Param(object):
    ...
    def mangle_to_epose(self, dt, pitch):  # forward compute
        ...
        '''
        s1 = self.m_angle_spd[1]
        s2 =  - self.m_angle_spd[0]
        damp_sita=  - (s2 / 2 + s1 / 2)
        dy =  math.sin(D_T_R * damp_sita) * self.damp_r
        dz = -math.cos(D_T_R * damp_sita) * self.damp_r
        if self.ground_state == 1 and 0 :
            self.spd_est1[0] = self.LPF(dy, self.spd_est1[0], 5, dt)
            self.spd_est1[1] = self.LPF(dz, self.spd_est1[1], 5, dt)
        '''
        return (y, z)

    def mangle_to_epose_spd(self, dt, pitch, pitch_rad):  # forward compute for speed
        l1 = self.l1
        l2 = self.l2
        s1 = self.to_180(self.m_angle[1] - 90)
        s2 = 270 - self.m_angle[0]
        ...
        r = self.damp_r
        d_sita1 = self.m_angle_spd[1] * D_T_R
        d_sita2 = -self.m_angle_spd[0] * D_T_R
        sqrt_all = math.sqrt(l2 ** 2 - (l1 ** 2) * math.sin(sita_cut) ** 2)
        d_sita_add = -(d_sita1 + d_sita2) / 2 + pitch_rad * D_T_R
        d_sita_sub = (d_sita2 - d_sita1) / 2
        dr = -l1 * math.sin(sita_cut * D_T_R) * d_sita_sub - (l1 ** 2) * math.sin(sita_cut) * math.cos(
            sita_cut) * d_sita_sub / sqrt_all

        dy = (math.cos(sita_add) * d_sita_add * r + math.sin(sita_add) * dr)
        dz = (math.sin(sita_add) * d_sita_add * r - math.cos(sita_add) * dr)
        FLT_END_SPD = 6
        if self.ground_state == 1 and 1:
            self.spd_est[0] = self.LPF(dy, self.spd_est[0], FLT_END_SPD, dt)
            self.spd_est[1] = self.LPF(dz, self.spd_est[1], FLT_END_SPD, dt)
        '''
        ...
        if self.ground_state == 1:
            ...
            if self.id == 2 and 0:
                print('ID: ', self.id)
                print('epos_spd_mot: ', dy, dz)
                print('epos_spd_div: ', dy_d, dz_d)
                print('epos_spd_flt: ', self.spd_est[0], self.spd_est[1])

        self.pos_est_r[0] = self.pos_est[0]
        self.pos_est_r[1] = self.pos_est[1]
        self.cnt_div = self.cnt_div + 1
        '''
        return (self.spd_est[0], self.spd_est[1])
    ...
    def vmc_control_test1(self, att, pos, dt):
        tar_spd = self.spd_cmd[0]
        est_spd = -self.spd_est[0]

        self.pos_cmd[0] = -(self.swing_pos_ed[1]) - self.COG_OFF
        self.pos_cmd[0] = -(tar_spd - est_spd) * 35
        err_y = self.limit(self.pos_cmd[0] - self.pos_est[0], -self.MAX_X * 0.86, self.MAX_X * 0.86)
        self.pos_cmd[
            1] = -0.2  # + -self.flag_rl*(0+ att[1])*0.003 + self.flag_fb*(0+ att[0])*0.003+ self.force_int[1] * 0

        err_z = self.limit(self.pos_cmd[1] - self.pos_est[1], -self.MAX_L * 0.86, self.MAX_L * 0.86)
        force_kp = [0, 0]
        force_kp[0] = self.limit(55 + math.fabs(math.sqrt(err_y ** 2)) * 400, 0, 66)
        force_kp[1] = self.limit(55 + math.fabs(math.sqrt(err_z ** 2)) * 400, 0, 66)
        force_kd = 600

        force_y = self.pos_cmd[0]  # err_y * force_kp[0] + (err_y - self.pos_ctrl_r[0]) * force_kd
        force_z = err_z * force_kp[1] + (err_z - self.pos_ctrl_r[1]) * force_kd

        self.force_cmd[1] = -force_y
        self.force_cmd[0] = force_z

        self.pos_ctrl_r[0] = err_y
        self.pos_ctrl_r[1] = err_z
    ...
    def vmc_control_test(self, att, rate, pos, dt):
        self.force_int[0] += dt * 2

        tar_spd = self.spd_cmd[0]
        est_spd = -self.spd_est[0]
        exp_pitch = -tar_spd * 25
        exp_roll = 0
        exp_height = -0.22 + math.sin(self.force_int[0]) * 0.035 * 0
        pid_roll = (exp_roll - att[1]) * self.flag_rl * 0.005 - (0 - rate[1]) * self.flag_rl * 0.005
        pid_pitch = (exp_pitch + att[0]) * self.flag_fb * 0.0025 - (0 + rate[0]) * self.flag_fb * 0.0025
        self.pos_cmd[1] = exp_height + pid_pitch * 1 + pid_roll * 1

        err_y = -self.limit(tar_spd - est_spd, -self.MAX_X * 0.86, self.MAX_X * 0.86)
        err_z = self.limit(self.pos_cmd[1] - self.pos_est[1], -self.MAX_L * 0.86, self.MAX_L * 0.86)
        force_kp = [0, 0]
        force_kd = [0, 0]
        force_kp[0] = 100
        ...
        force_y = err_y * force_kp[0] + (err_y - self.pos_ctrl_r[0]) * force_kd[0]
        force_z = err_z * force_kp[1] + (err_z - self.pos_ctrl_r[1]) * force_kd[1]
...
class Brain_Param(object):
    ...
    def state_est(self, leg1=Leg_Param, leg2=Leg_Param, leg3=Leg_Param, leg4=Leg_Param):
        self.ground_num = leg1.ground_state + leg2.ground_state + leg3.ground_state + leg4.ground_state
        self.pos_n[2] = math.fabs(
    ...
    def spd_control(self, dt, leg1=Leg_Param, leg2=Leg_Param, leg3=Leg_Param, leg4=Leg_Param):
        err_yaw_spd = self.limit((0 - self.rate[2]) * R_T_D, -66, 66)
        out_pid_yaw = err_yaw_spd * 0.00
        err =
        self.tar_spd_use[L] = self.LPF(self.exp_spd_b[1] + out_pid_yaw, self.tar_spd_use[L], 0.25, dt)
        self.tar_spd_use[R] = self.LPF(self.exp_spd_b[1] - out_pid_yaw, self.tar_spd_use[R], 0.25, dt)
        # print(out_pid_yaw,self.tar_spd_use[L],self.tar_spd_use[R])
