...
class Minitaur(object):
  ...
  def ApplyAction_T(self, motor_commands):
      torque_limit = OVERHEAT_SHUTDOWN_TORQUE
      motor_commands = np.clip(motor_commands, -torque_limit, torque_limit)

      if self._motor_overheat_protection:
          for i in range(self.num_motors):
              if abs(motor_commands[i]) > OVERHEAT_SHUTDOWN_TORQUE:
                  self._overheat_counter[i] += 1
              else:
                  self._overheat_counter[i] = 0
              if (self._overheat_counter[i] > OVERHEAT_SHUTDOWN_TIME / self.time_step):
      ...
      self._observed_motor_torques = motor_commands
