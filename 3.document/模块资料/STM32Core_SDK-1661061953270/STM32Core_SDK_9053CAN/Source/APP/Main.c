...
USB-TTL         STM32Core                     HWT9053-CAN
  ...
  GND  <----->    GND           <---------->      GND      			 					
								   ...
								   TTL-CAN
               CAN_TX(GPIOA_12) --TX     CANH --- CANH
			   CAN_RX(GPIOA_11) --RX     CANL --- CANL
				  GND      -----     GND      ---  GND
-----------------------------------------------------------------------
*/
#include <string.h>
...
const uint32_t c_uiBaud[15] = {1000000, 800000 , 500000, 400000, 250000, 200000, 125000, 100000, 80000, 50000, 40000, 20000, 10000, 5000 , 3000};
	...
	while (1)
	{
		...
		if(s_cDataUpdate)
		{
			for(i = 0; i < 3; i++)
			{
				...
				iTemp =(((uint32_t)sReg[HRoll + 2*i]) << 16) | ((uint16_t)sReg[LRoll + 2*i]);
				fAngle[i] = (float)iTemp / 1000.0f;
			}
...
static void ShowHelp(void)
{
	...
	printf("UART SEND:U\\r\\n   Bandwidth increase.\r\n");
	printf("UART SEND:u\\r\\n   Bandwidth reduction.\r\n");
	printf("UART SEND:B\\r\\n   Baud rate increased to can rate :500000.\r\n");
	printf("UART SEND:b\\r\\n   Baud rate reduction to can rate :250000.\r\n");
