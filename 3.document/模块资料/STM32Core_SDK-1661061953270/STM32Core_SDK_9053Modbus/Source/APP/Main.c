...
USB-TTL            STM32Core              485 adapter board 	    HWT9053-485
 VCC    <------>     VCC          <---->      	VCC			 <----->    VCC
 TX     <------>   RX1(GPIOA_10)   
 RX     <------>   TX1(GPIOA_9)													  
 GND    <------>     GND          <---->  		GND          <----->    GND
...
static void CmdProcess(void)
{
	switch(s_cCmd)
	{
		...
		case 'm':	
			...
			break;
		case 'S': 
			if(WitSaveParameter(SAVE_PARAM) != WIT_HAL_OK) 
				printf("\r\nSet Parameter Error\r\n");
            break;
		case 'e':	
	    ...
	    case 'U':	
			...
			break;
		case 'H':	
			if(WitSetBandwidth(BANDWIDTH_21HZ) != WIT_HAL_OK) 
				printf("\r\nSet Bandwidth Error\r\n");
			break;
		case 'u':	
